...
module.exports = (bot, utils, ytdl, config) => {
    ...
    bot.handleVideo = async (video, message, vc, playlist = false) => {
        ...
        if (!queue) {
            ...
            if (playlist) return;
            else return message.channel.send(`:eye: **${music.title}** has been added to queue :smirk:`);
        }
        return;
    }
    ...
    bot.play = (guild, music) => {
        ...
        if (!music) {
            ...
            return queue.textChannel.send(`:eye: See you again next time BYE! :stuck_out_tongue_winking_eye: `);
        }
        ...
        queue.textChannel.send(`:eye: **${music.title}** is now being played :musical_note:`);
    }
